tvar <- function(mydata,lags=1,thMax,thresh=1,tarscale=0.5,tarstandard=NULL,intercept=TRUE,RandomWalk=TRUE,coefprior=NULL,coefpriorvar=1,varprior=1,varpriordof=1,irfhorizon=16,irfquantiles=c(0.05,0.95),reps=300,burnin=100,stabletest=FALSE){
  y <- as.matrix(mydata)
  T <- nrow(y)
  K <- ncol(y)
  ...
  results <- .tigibbs(y,lags,thMax,thresh,tarscale,tarstandard,intercept,prior$Aprior1,prior$Aprior2,prior$Vprior1,prior$Vprior2,prior$Sprior1,prior$Sprior2,varpriordof,irfhorizon=16,irfquantiles=c(0.1,0.9),reps=210,burnin=10,stabletest=FALSE)
